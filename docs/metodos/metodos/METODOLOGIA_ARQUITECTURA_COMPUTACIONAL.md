...
│  │  │                               │  │    │  │   Stack: 4096 bytes                 │    │ │
│  │  │   Cada 10 ms:                 │  │    │  │                                     │    │ │
│  │  │   • nextion.process()         │  │    │  │   Bucle continuo:                   │    │ │
│  │  │   • Parsear eventos táctiles  │  │    │  │   • Verificar estado RUNNING        │    │ │
│  │  │   • stateMachine.update()     │  │    │  │   • Calcular espacio en buffer      │    │ │
│  │  │                               │  │    │  │   • Generar muestras (1 kHz)        │    │ │
│  │  │   Cada WAVEFORM_UPDATE_MS:    │  │    │  │   • Escribir en buffer circular     │    │ │
│  │  │   • updateDisplay()           │  │    │  │   • vTaskDelay(1) yield             │    │ │
│  │  │   • addWaveformPoint()        │  │    │  └─────────────────────────────────────┘    │ │
│  │  │                               │  │    │                    │                        │ │
│  │  │   Cada METRICS_UPDATE_MS:     │  │    │                    ▼                        │ │
│  │  │   • updateMetrics()           │  │    │  ┌─────────────────────────────────────┐    │ │
│  │  │   • updateScaleLabels()       │  │    │  │   Modelos de Señal                  │    │ │
│  │  └───────────────────────────────┘  │    │  │                                     │    │ │
...
│  │  │                               │  │    │  │                                     │    │ │
│  │  │   Cada WS_METRICS_INTERVAL:   │  │    │  │   writeIndex ──► [████████░░░░]     │    │ │
│  │  │   • sendMetrics() (4 Hz)      │  │    │  │                   readIndex ──┘     │    │ │
...
│  ├───────────────────────┤   ├───────────────────────┤   ├─────────────────────────────┤   │
│  │                       │   │                       │   │                             │   │
│  │  Actualización: 1 kHz │   │  Waveform: 100 Hz     │   │  Señal: 100 Hz (10 ms)      │   │
...
| Core 1 | Timer ISR | Salida DAC | 1 ms | Máxima | ISR |
...
| `SAMPLE_RATE_HZ` | 1000 | Tasa de actualización DAC |
...
El timer de hardware se configuró para generar interrupciones a 1 kHz exactos:
...
void SignalEngine::setupTimer() {
    ...
    timerAlarmWrite(signalTimer, 1000, true);  // 1000 µs = 1 kHz
...
El buffer circular de 2048 muestras proporcionó aproximadamente 2 segundos de autonomía a 1 kHz, suficiente para absorber variaciones temporales de la tarea de generación:

**Cálculo de autonomía del buffer:**
$$T_{buffer} = \frac{N_{muestras}}{f_s} = \frac{2048}{1000\,Hz} = 2.048\,s$$
...
Dado que los modelos generaron muestras a tasas superiores a la visualización (excepto PPG), se implementó downsampling por decimación:
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Modelo ECG (750 Hz) ──┬──► Decimación 7.5:1 ──► Nextion 100 Hz │
│                        │                                        │
│  Modelo EMG (2000 Hz) ─┼──► Decimación 20:1 ───► Nextion 100 Hz │
│                        │                                        │
│  Modelo PPG (100 Hz) ──┴──► Sin decimación ────► Nextion 100 Hz │
│                                                                 │
│  Implementación:                                                │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  // En loop(), cada WAVEFORM_UPDATE_MS (10 ms):         │    │
│  │  if (millis() - lastWaveformUpdate >= WAVEFORM_UPDATE_MS) {  │
│  │      uint8_t value = model.getWaveformValue();          │    │
│  │      nextion.addWaveformPoint(WAVEFORM_ID, 0, value);   │    │
│  │      lastWaveformUpdate = millis();                     │    │
│  │  }                                                      │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                 │
│  Notas:                                                         │
│  • El downsampling usa la última muestra disponible             │
│  • Para EMG se envían 2 canales: raw + envolvente               │
│  • La tasa efectiva de envío es 100 puntos/segundo              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| Señal | fs modelo | fs display | Ratio | Muestras/s enviadas |
|-------|-----------|------------|-------|---------------------|
| ECG | 750 Hz | 100 Hz | 7.5:1 | 100 |
| EMG | 2000 Hz | 100 Hz | 20:1 | 200 (2 canales) |
| PPG | 100 Hz | 100 Hz | 1:1 | 100 |
...
│  │  (ISR 1 kHz)  │      │  (100 Hz)     │      │  (100 Hz)     │           │
│  ├───────────────┤      ├───────────────┤      ├───────────────┤           │
│  │               │      │               │      │               │           │
│  │ timerISR():   │      │ updateDisplay │      │ sendSignal    │           │
│  │ dacWrite()    │      │ ()            │      │ Data()        │           │
│  │               │      │               │      │               │           │
│  │ Cada 1 ms     │      │ Cada 10 ms    │      │ Cada 10 ms    │           │
...
| Buffer → DAC | 1 ms (fijo) | 1 ms + jitter (±50 µs) |
...
|--------|-------|--------------|---------|-------|
| Core 1 | generationTask | ~0.8 ms | 1 ms | ~80% |
| Core 1 | Timer ISR | ~5 µs | 1 ms | ~0.5% |

...
## 21. Diagrama de Flujo General del Sistema

### 21.1 Arquitectura del Sistema Completo

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    BIOSIMULATOR PRO - ARQUITECTURA GENERAL                   │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                         CAPA DE USUARIO                              │    │
│  │  ┌───────────────────┐                    ┌───────────────────────┐ │    │
│  │  │   NEXTION DISPLAY │◄──────────────────►│    TERMINAL SERIAL    │ │    │
│  │  │     (320x240)     │     Visualización  │     (Debug/Config)    │ │    │
│  │  └─────────┬─────────┘                    └───────────┬───────────┘ │    │
│  └────────────┼──────────────────────────────────────────┼─────────────┘    │
│               │ UART 115200                              │ USB 115200       │
│  ┌────────────┼──────────────────────────────────────────┼─────────────┐    │
│  │            ▼                                          ▼             │    │
│  │  ┌─────────────────────────────────────────────────────────────┐   │    │
│  │  │                    CAPA DE COMUNICACIÓN                      │   │    │
│  │  │  ┌───────────────────┐          ┌───────────────────────┐   │   │    │
│  │  │  │  NextionDriver    │          │    SerialHandler      │   │   │    │
│  │  │  │  • parseEvent()   │          │    • processCmd()     │   │   │    │
│  │  │  │  • goToPage()     │          │    • sendResponse()   │   │   │    │
│  │  │  │  • addWaveform()  │          │                       │   │   │    │
│  │  │  └─────────┬─────────┘          └───────────┬───────────┘   │   │    │
│  │  └────────────┼────────────────────────────────┼───────────────┘   │    │
│  │               │                                │                   │    │
...
│  │  │  │                   CAPA DE CONTROL                    │   │   │    │
│  │  │  │  ┌───────────────┐        ┌───────────────────────┐ │   │   │    │
│  │  │  │  │ StateMachine  │◄──────►│   ParamController     │ │   │   │    │
│  │  │  │  │ • PORTADA     │        │   • validateParam()   │ │   │   │    │
│  │  │  │  │ • MENU        │        │   • setCondition()    │ │   │   │    │
│  │  │  │  │ • SELECT_COND │        │   • getLimit()        │ │   │   │    │
│  │  │  │  │ • SIMULATING  │        │                       │ │   │   │    │
│  │  │  │  │ • PAUSED      │        │                       │ │   │   │    │
│  │  │  │  └───────┬───────┘        └───────────┬───────────┘ │   │   │    │
│  │  │  └──────────┼────────────────────────────┼─────────────┘   │   │    │
...
│  │  │  │                    ESP32 WROOM-32                  │   │   │    │
│  │  │  │  ┌─────────────┐  ┌─────────────┐  ┌───────────┐  │   │   │    │
│  │  │  │  │ GPIO25 DAC  │  │ GPIO16/17   │  │ GPIO 4,5  │  │   │   │    │
│  │  │  │  │   Output    │  │  UART2      │  │ LED RGB   │  │   │   │    │
│  │  │  │  └──────┬──────┘  └──────┬──────┘  └─────┬─────┘  │   │   │    │
│  │  │  └─────────┼────────────────┼───────────────┼────────┘   │   │    │
│  │  └────────────┼────────────────┼───────────────┼────────────┘   │    │
│  └───────────────┼────────────────┼───────────────┼────────────────┘    │
│                  ▼                ▼               ▼                     │
│         ┌────────────────┐ ┌──────────────┐ ┌──────────────┐            │
│         │ SEÑAL ANALÓGICA│ │   NEXTION    │ │  LED STATUS  │            │
│         │   0-3.3V       │ │   NX4024T032 │ │  RGB         │            │
│         │   @ 1kHz       │ │   320x240    │ │              │            │
│         └────────────────┘ └──────────────┘ └──────────────┘            │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
...
### 21.2 Flujo Principal del Sistema

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         FLUJO PRINCIPAL (main.cpp)                           │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │                              SETUP()                                   │  │
...
│                                      ▼                                       │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ 1. Inicializar Serial Debug (115200)                                  │  │
│  │ 2. Inicializar LED RGB                                                │  │
│  │ 3. Inicializar NextionDriver (Serial2, 115200)                       │  │
│  │ 4. Inicializar SerialHandler                                          │  │
│  │ 5. Inicializar SignalEngine (crear tarea Core 1)                     │  │
│  │ 6. Configurar StateMachine callbacks                                  │  │
│  │ 7. Enviar evento INIT_COMPLETE → Estado = PORTADA                    │  │
│  └───────────────────────────────────┬───────────────────────────────────┘  │
│                                      │                                       │
│                                      ▼                                       │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
...
│         ▼                            ▼                            ▼         │
│  ┌──────────────┐         ┌───────────────────┐         ┌──────────────┐   │
│  │ nextion->    │         │  serialHandler->  │         │ updateDisplay│   │
│  │ process()    │         │  process()        │         │ ()           │   │
│  │              │         │                   │         │              │   │
│  │ Lee eventos  │         │ Lee comandos      │         │ Cada 10ms:   │   │
│  │ touch del    │         │ del terminal      │         │ Envía punto  │   │
│  │ display      │         │ debug             │         │ al waveform  │   │
│  └──────┬───────┘         └─────────┬─────────┘         │              │   │
│         │                           │                   │ Cada 250ms:  │   │
│         │                           │                   │ Actualiza    │   │
│         │                           │                   │ métricas     │   │
...
│  │  ┌────────────────────────────────────────────────────────────┐    │    │
│  │  │ PORTADA ──(COMENZAR)──► MENU                               │    │    │
│  │  │ MENU ──(SELECT_*)──► SELECT_CONDITION                      │    │    │
│  │  │ SELECT_CONDITION ──(GO_TO_WAVEFORM)──► SIMULATING          │    │    │
│  │  │ SIMULATING ──(PAUSE)──► PAUSED                             │    │    │
│  │  │ SIMULATING ──(STOP)──► MENU                                │    │    │
│  │  └────────────────────────────────────────────────────────────┘    │    │
│  └───────────────────────────────┬────────────────────────────────────┘    │
│                                  │                                          │
│                                  ▼                                          │
...
│  │  ┌────────────────────────────────────────────────────────────┐    │    │
│  │  │ Si newState == SIMULATING:                                  │    │    │
│  │  │   → nextion->goToPage(ECG_WAVE/EMG_WAVE/PPG_WAVE)          │    │    │
│  │  │   → signalEngine->startSignal(type, condition)             │    │    │
│  │  │   → setLEDState(RUNNING)                                   │    │    │
│  │  └────────────────────────────────────────────────────────────┘    │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
...
### 21.3 Flujo de Generación de Señal (Core 1)
...
│                                      ▼                                       │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ 1. Detener señal anterior (si existe)                                 │  │
│  │ 2. Reset buffers circulares                                           │  │
│  │ 3. Configurar modelo según tipo:                                      │  │
│  │    • ECG: ECGParameters, condición 0-8                                │  │
│  │    • EMG: EMGParameters, condición 0-9                                │  │
│  │    • PPG: PPGParameters, condición 0-6                                │  │
│  │ 4. Pre-llenar buffer                                                  │  │
│  │ 5. Iniciar timer @ 1000 Hz                                            │  │
│  │ 6. Estado = RUNNING                                                   │  │
│  └───────────────────────────────────┬───────────────────────────────────┘  │
│                                      │                                       │
│                                      ▼                                       │
...
│                     ▼                                 ▼                     │
│  ┌─────────────────────────────────┐   ┌─────────────────────────────────┐ │
│  │      WHILE (RUNNING)            │   │      TIMER ISR @ 1000 Hz        │ │
│  │  ┌───────────────────────────┐  │   │  ┌───────────────────────────┐  │ │
│  │  │ ¿Buffer tiene espacio?    │  │   │  │ Leer valor del buffer     │  │ │
│  │  │          │                │  │   │  │ circular                  │  │ │
│  │  │          ▼                │  │   │  │          │                │  │ │
│  │  │ [SÍ]: Generar 10 muestras │  │   │  │          ▼                │  │ │
│  │  │       y añadir al buffer  │  │   │  │ Escribir al DAC (GPIO25)  │  │ │
│  │  │                           │  │   │  │ lastDACValue = valor      │  │ │
│  │  │ [NO]: esperar 1ms         │  │   │  │                           │  │ │
│  │  └───────────────────────────┘  │   │  └───────────────────────────┘  │ │
│  └─────────────────────────────────┘   └─────────────────────────────────┘ │
...
│  │  │  switch(signalType):                                          │  │   │
│  │  │                                                               │  │   │
│  │  │    ECG: dacValue = ecgModel.getDACValue()                     │  │   │
│  │  │          • RK4 integration                                    │  │   │
│  │  │          • 5 gaussian waves (P,Q,R,S,T)                       │  │   │
│  │  │          • Variability                                        │  │   │
│  │  │                                                               │  │   │
│  │  │    EMG: dacValue = emgModel.getDACValue()                     │  │   │
│  │  │          • 100 Motor Units                                    │  │   │
│  │  │          • Recruitment + Rate coding                          │  │   │
│  │  │          • MUAP summation                                     │  │   │
│  │  │                                                               │  │   │
│  │  │    PPG: dacValue = ppgModel.getDACValue()                     │  │   │
...
### 21.4 Flujo de Visualización (Core 0)
...
│         ▼                            ▼                            ▼         │
│  ┌──────────────────┐     ┌───────────────────┐     ┌──────────────────┐   │
│  │ WAVEFORM UPDATE  │     │  METRICS UPDATE   │     │   LED UPDATE     │   │
│  │  (cada 10ms)     │     │  (cada 250ms)     │     │                  │   │
│  │                  │     │                   │     │                  │   │
│  │ ¿signalEngine    │     │ ¿signalEngine     │     │ Según estado:    │   │
│  │ está RUNNING?    │     │ está RUNNING?     │     │ • RUNNING: Verde │   │
│  └────────┬─────────┘     └─────────┬─────────┘     │ • PAUSED: Azul   │   │
│           │                         │               │ • ERROR: Rojo    │   │
│  [SÍ]     │                [SÍ]     │               └──────────────────┘   │
│           ▼                         ▼                                       │
│  ┌──────────────────┐     ┌───────────────────┐                            │
│  │ dacValue =       │     │ Leer métricas     │                            │
│  │ signalEngine->   │     │ del modelo:       │                            │
│  │ getLastDACValue()│     │                   │                            │
│  │                  │     │ ECG: HR, RR       │                            │
│  │ waveValue =      │     │ EMG: RMS, MUs     │                            │
│  │ map(dacValue,    │     │ PPG: HR, PI       │                            │
│  │     0,255,       │     │                   │                            │
│  │     10,201)      │     │                   │                            │
│  └────────┬─────────┘     └─────────┬─────────┘                            │
│           │                         │                                       │
...
│  ┌──────────────────┐     ┌───────────────────┐                            │
│  │ nextion->        │     │ nextion->         │                            │
│  │ addWaveformPoint │     │ updateMetrics     │                            │
│  │ (1, 0, waveValue)│     │ (metrics, type)   │                            │
│  │                  │     │                   │                            │
│  │ Envía:           │     │ Envía:            │                            │
│  │ "add 1,0,150"    │     │ "val_hr.val=72"   │                            │
